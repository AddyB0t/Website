...
import { Book as BookType, Subject } from "@/lib/supabase"
...
export default function SubjectPage({ params }: { params: { subjectId: string } }) {
  ...
  const fetchData = async () => {
    setIsLoading(true)
    try {
      // Fetch subject info from subjects API
        ...
        <div className="max-w-7xl mx-auto">
          {isLoading ? (
            <div className="flex items-center justify-center py-12">
              ...
              <span className="text-gray-600">Loading subject data...</span>
            </div>
          ) : (
            <>
              <div className="mb-8">
                <h1 className="text-3xl font-light text-gray-800 mb-2">{subject?.name}</h1>
                <p className="text-gray-600">{subject?.description || 'Select a book to explore its content'}</p>
              </div>
                ...
                {books.length > 0 ? (
                  books.map((book) => (
                    <Card key={book.id} className="overflow-hidden hover:shadow-md transition-shadow">
                      <Link href={`/classroom/${params.subjectId}/${book.id}`}>
                        <div className="h-40 bg-blue-100 flex items-center justify-center">
                          <Book className="w-16 h-16 text-blue-600 opacity-80" />
                        </div>
                        <CardContent className="p-6">
                          <h2 className="text-xl font-medium text-gray-800 mb-2">{book.title}</h2>
                          {book.publisher && (
                  ...
                  <div className="col-span-full bg-white rounded-xl shadow-sm border border-gray-100 p-8 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-gray-600 max-w-md">
                      There are no books available for this subject at the moment. Please check back later.
                    </p>
                  </div>
                )}
              </div>
